...
process.stdin.on('end', () => {
  try {
    ...
    const isInAdapterSchemas = filePath.includes('/adapters/') && filePath.includes('/schemas/');
    ...
    for (const match of enumMatches) {
      ...
      if (isInDomainSchemas && /[a-z][A-Z]/.test(values) && !values.includes('_')) {
        warnings.push('Domain schemas should use snake_case (e.g., indoor_cycling)');
      }
      if (isInAdapterSchemas && /_[a-z]/.test(values)) {
        warnings.push('Adapter schemas should use camelCase (e.g., indoorCycling)');
      }
    }
