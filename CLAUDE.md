...
- ✅ **Mechanical guards passing** - `pnpm test:scripts` enforces:
  ...
  - `check-model-ids-fresh.mjs` — every model id written by hand into `packages/ai/{README,AGENTS}.md` or `.github/workflows/eval.yml` MUST exist in the generated `MODEL_CATALOG`, or be listed in `HISTORICAL` with a reason (R-ModelIdsFresh). The catalog is regenerated from the installed `@ai-sdk/*` type unions and cannot drift silently; an id typed into prose has no such backstop, and `claude-sonnet-4-5-20241022` — an id that never existed — sat in four documents. A catalog parsing to zero ids is a FAULT
  - `check-no-inert-floors.mjs` — a runner taking its model from `load-eval-model` cannot execute without a provider key, so no score of its may decide its exit code (R-NoInertFloors). Two rules, because one is evadable: `process.exit` takes a numeric literal, AND no comparison may be applied to a score-shaped name. The plain grep is defeated by `const ok = rate >= 0.9; process.exit(ok ? 0 : 1)`. A KEYLESS runner is exempt and may gate — a directory-wide ban would forbid the gating runner this project should eventually want
  - `check-untrusted-fields-fenced.mjs` — a recognized externally-authored field (`name`, `title`, `description`, `notes`, `error`, `message`, `reason`) returned to the model from `application/chat/tools/**` MUST be assigned from `fenceUntrusted(...)`; the guard names the file and the field (R-UntrustedFieldsFenced). It is syntactic: a field under a name outside the set is a KNOWN gap, and an opaque nested payload is invisible to it. Values built only from string literals, and zod schema builders, are app-authored by construction and exempt
  - `check-model-facing-sinks.mjs` — every chat tool whose `execute` returns a value the model reads MUST appear in `scripts/model-facing-sinks.mjs` with a status (`clean` / `fenced` / `unbounded`) and a provenance note naming where its strings are produced (R-ModelFacingSinksDeclared). A field-name allowlist catches the fields you named; this catches the sink you forgot, which is the failure that actually happened — a summarizer passed `workout.name` through raw while the fence docstring claimed it was covered, and a coaching bridge's exception text reached the model through a field called `error`, outside the tools directory, with no summarizer involved. Zero files scanned, an unreadable directory, and an unreadable source are FAULTS, not passes
