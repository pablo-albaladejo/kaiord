...
| `check-no-library-dual-mount.mjs`   | No-dual-mount invariant for the Library content component (spec/spa-routing). Fails if any non-allowlisted file imports `organisms/WorkoutLibrary`, `WorkoutLibrary/WorkoutLibrary`, or `LibraryDialogContent`. Allowlist limited to LibraryPage and TemplatePickerDialog. | `pnpm lint:no-library-dual-mount` (→ `pnpm lint`)   | `check-no-library-dual-mount.test.mjs` (6 cases) |
