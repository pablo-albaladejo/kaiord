...
| `check-specs-inventory.mjs`                               | Drift guard for `openspec/specs/README.md`.                                           | `pnpm lint:specs-inventory`                                                   |
| `check-bridge-privacy-surface.mjs`                        | Locks every bridge's manifest, allowlist, actions and auth endpoints vs a golden.     | `pnpm test:scripts`                                                           |
| `check-bridge-stale-threshold-parity.test.mjs`            | Test-only parity invariant on bridge stale thresholds.                                | `pnpm test:scripts`                                                           |
| `check-bridge-popup-message-parity.test.mjs`              | Test-only parity between each bridge popup's fallback table and its en catalog.       | `pnpm test:scripts`                                                           |
| `check-bridge-popup-tokens-parity.test.mjs`               | Test-only parity between the popup shell's `--kd-*` palette and `brand-tokens.css`.   | `pnpm test:scripts`                                                           |
...
| `check-discovery-clock-reset.mjs`                         | `R-DiscoveryClockReset` — a test reaching `useDiscoverySettled` MUST place the clock. | `pnpm lint:discovery-clock-reset`                                             |
| `check-frozen-hex-parity.mjs`                             | `R-FrozenHexParity` — every frozen hex mirror still equals the role it mirrors.       | `pnpm test:scripts`                                                           |
...
| `check-model-ids-fresh.mjs`                               | `R-ModelIdsFresh` — a model id written in prose exists in MODEL_CATALOG.              | `pnpm lint:model-ids-fresh`, `pnpm test:scripts`                              |
| `check-no-inert-floors.mjs`                               | `R-NoInertFloors` — a credential-bound runner may not gate on a score.                | `pnpm lint:no-inert-floors`, `pnpm test:scripts`                              |
| `check-model-facing-sinks.mjs`                            | `R-ModelFacingSinksDeclared` — every chat tool the model reads is in the inventory.   | `pnpm lint:model-facing-sinks`, `pnpm test:scripts`                           |
| `check-no-barrel-test-suites.mjs`                         | `R-NoBarrelTestSuite` — no test suite over a pure re-export barrel.                   | `pnpm test:scripts`, husky `pre-commit` (`--changed-files`)                   |
...
| `check-no-raw-chromatic.mjs`                              | `R-NoRawChromatic` — no raw chromatic Tailwind utility in the SPA; name a role.       | `pnpm test:scripts`                                                           |
...
| `check-untrusted-fields-fenced.mjs`                       | `R-UntrustedFieldsFenced` — a recognized external field must go through the fence.    | `pnpm lint:untrusted-fields-fenced`, `pnpm test:scripts`                      |
...
| `generate-specs-inventory.mjs`                            | Render the specs inventory `README.md`.                                               | `pnpm specs:inventory` (and reused by `check-specs-inventory.mjs`)            |
