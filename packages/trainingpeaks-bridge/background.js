...
// prettier-ignore
// The `\/` inside the character classes is redundant to the regex engine and
// load-bearing to us. `extractPatternAllowlist` scans forward from `pattern: /`
// and breaks on the first UNESCAPED `/`, with no notion of character classes,
// so `[^/]` truncates the recorded pattern at `[^` — and the guard still
// passes, because the golden is regenerated by the same extractor. Measured:
// normalising these produced a golden entry ending `consolidatedtimedmetrics\/[^`.
