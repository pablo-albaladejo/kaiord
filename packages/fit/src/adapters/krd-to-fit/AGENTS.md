...
| `krd-to-fit-workout.mapper.ts`         | Maps KRD steps to FIT workout step messages; handles repetitions.                           |
| `krd-to-fit-duration.mapper.ts`        | Maps KRD step duration to FIT duration fields (type, value, time, distance, HR/power zone). |
| `krd-to-fit-target.converter.ts`       | Maps KRD step target to FIT target fields (type, value, zones).                             |
| `krd-to-fit-step.mapper.ts`            | Maps individual KRD step properties (name, intensity, equipment).                           |
...
- **Repetition expansion:** KRD steps may have repetitions (e.g., 5x [warm, main, cool]). `krd-to-fit-workout.mapper.ts` expands these into flat step arrays with correct message indices.
...
- **Mappers chain:** `krd-to-fit.converter.ts` → `krd-to-fit-metadata.converter.ts` → `krd-to-fit-workout.mapper.ts` → duration/target converters.
- **Duration dispatch:** `krd-to-fit-duration.mapper.ts` routes to type-specific converter in `duration-converters/`.
