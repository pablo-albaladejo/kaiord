...
| Dialogs      | ProfileEditDialog (tabs: Training Zones / Personal Data / Linked Accounts / Data Flows; nested DeleteConfirmDialog), ThresholdEditDialog, disconnect ConfirmationModal                      |
...
| DataFlowsAddDialog (removed)                       | _deleted (was `organisms/ProfileManager/components/DataFlowsAddDialog.tsx`)_ | None — retired in F4.2: creating a route is now exclusively the Data Hub matrix's job (`/settings/data-hub`); the Data Flows tab is read/edit-only (mode, enabled, remove)       | Add a sync source/destination (bridge + mode)                              | n/a (removed)                                          |
...
- **DataFlowsAddDialog retired (F4.2):** the Data Flows tab's "+ Add source/destination" dialog (`useDataFlowsAdd`/`DataFlowsAddDialog`/`DataFlowsAddFields`) was removed once the Data Hub matrix (`/settings/data-hub`) covered the same create-by-(dataType, bridge, direction) surface. The Data Flows tab is now read/edit-only (mode, enabled, remove via `DataFlowsRow`). AthleteConnections' per-connection "What syncs" toggles (`ConnectionFlows`/`FlowRow`/`flow-availability.ts`/`use-bridge-capabilities.ts`) were retired the same way — `ConnectedRow` now only links to the matrix.
