...
| `/settings/:section?` | `SettingsPage` (`components/pages/SettingsPage/SettingsPage.tsx`)    | `section?` ∈ ai\|sync\|data-hub\|extensions\|usage\|privacy\|preferences | No section → grouped index. Section → index becomes a `md:` rail beside the panel. Unknown section → **Redirect `/settings`** |
...
| Renders         | `components/pages/SettingsPage/SettingsPage.tsx` (grouped index when no section; section organism from `settings-tab-views.tsx` registry)                                                                                                                                                                                                                                                             |
...
| Bottom nav      | Visible (mobile)                                                                                                                                                                                                                                                                                                                                                                                      |
| Responsive      | **Yes, from `md`** — with a section open the container is `md:grid md:grid-cols-[18rem_minmax(0,1fr)]`: the index becomes a `hidden md:block` rail (labels only, `aria-current` on the open section) beside the panel. CSS only; no viewport is measured. Not sticky (see the change's design.md D9)                                                                                                  |
...
- **Routing ground truth verified** against `AppRoutes.tsx`: all 12 routes present and correctly attributed, including the `/calendar` → `/calendar/:weekId` week-grid redirect and the `/settings/profile` → `/athlete` redirect being declared _above_ `/settings/:section?` so it wins. The Health sub-router's 6 lazy pages (including `/health/labs`, added for the lab-analytics DoD-1 entry form) + null→Redirect fallback are all mapped.
