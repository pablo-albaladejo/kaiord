...
| Dialogs      | ScheduleDateDialog (normal flow), delete ConfirmationModal. (TemplatePickerDialog is **not** opened by this page)                                        |
...
| TemplatePickerDialog                               | `molecules/TemplatePickerDialog/TemplatePickerDialog.tsx`               | NewWorkoutPicker (orphan, w/ date), AddEntryChooser, WellnessEntryDialog | Search-only in-flow picker bound to a date                                 | **Yes** — pick → `/calendar` (via picker schedule)     |
...
| TemplatePickerDialog                       | **Yes**                         | Picker list `grid-cols-1` → `sm:grid-cols-2`                                                                                                        | sm                 |
