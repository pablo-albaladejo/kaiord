...
- **Routing ground truth verified** against `AppRoutes.tsx`: all 12 routes present and correctly attributed, including the `/calendar` → `/calendar/:weekId` week-grid redirect and the `/settings/profile` → `/athlete` redirect being declared _above_ `/settings/:tab?` so it wins. The Health sub-router's 6 lazy pages (including `/health/labs`, added for the lab-analytics DoD-1 entry form) + null→Redirect fallback are all mapped.
