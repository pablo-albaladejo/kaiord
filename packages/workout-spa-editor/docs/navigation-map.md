...
| `/settings/:section?`    | `SettingsPage` (`components/pages/SettingsPage/SettingsPage.tsx`)    | `section?` ∈ ai\|sync\|connections\|usage\|privacy\|preferences | No section → grouped index. Section → `md:` section rail beside the panel. Retired `data-hub`/`extensions` → **Redirect (replace) `/settings/connections`**. Any other unknown section → **Redirect `/settings`** |
...
graph LR
  ...
  STATUSLABS["StatusHeader Labs entry"] -->|navigate /health/labs| HL
...
- **LayoutHeader** (`LayoutHeader.tsx`) — **sticky top-0 z-50**, present on every route. Contains `HeaderLogo` (brand `<Link href="/calendar">`, left) + `StatusHeader` (right): primary nav buttons (Calendar → `/calendar/:weekId`, Library → `/library`, Trends → `/health`, New → `/workout/new`), `StatusIndicators` (Garmin / Train2Go pills, display-only), `ProfileEntryButton` (→ `/athlete`), Settings (→ `/settings`), and `ThemeToggle`. There is **no Help button** — help is keyboard-first: `useHeaderOverlays` mounts the ⌘K command palette and the `?` shortcut sheet, and long-form documentation lives on the docs site. Responsive: inner bar is `flex-col items-center gap-2` on mobile, `sm:flex-row sm:gap-4` on desktop; entry-button labels are `hidden sm:inline` (only "New workout" keeps its label on mobile); the nav↔indicator divider is `hidden sm:inline-block`.
...
| Bottom nav   | Visible (mobile)                                                                                                                                                                                         |
| Responsive   | **No distinct layouts** — single column (`space-y-5 px-4`); grep-confirmed zero responsive prefixes / no useMediaQuery / no grid-cols across AthletePage                                                 |
| Sub-views    | loading / empty / body; sport Segmented (cycling\|running\|swimming, in-page useState). The page is identity + thresholds + zones only: the Connections section was retired into `/settings/connections` |
| Dialogs      | ProfileEditDialog (tabs: Training Zones / Personal Data / Linked Accounts; nested DeleteConfirmDialog), ThresholdEditDialog                                                                              |
| Outgoing nav | AthleteEmptyState "Create profile" opens `ProfileManagerDialog` in place (no navigation). **No route links** — the page navigates nowhere                                                                |
...
|                 |                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Route           | `/settings/:section?` (section ∈ ai\|sync\|connections\|usage\|privacy\|preferences; `data-hub` and `extensions` are retired aliases that redirect to `connections`)                                                                                                                                                                                                                                                              |
...
| Sub-views       | Index groups (Your data / AI / Preferences / About); sections: AI (ProviderList + inline ProviderEditRow + ProviderForm + custom prompt), Sync (status line + Connect/Sync/Disconnect + EncryptionSection), Connections (health counters + consequence banner + source cards + the 13 data-type routing rows), Usage (6-month table), Privacy (info + Clear All API Keys, no confirm), Preferences (units/language/notifications) |
| Attention slots | `SettingsAttention` banner (under the h1) and chip (top of the rail), fed by `useSettingsAttention` over the bridge-connection model. Index carries the banner, a section carries the chip — never both; both render nothing while every connection is healthy                                                                                                                                                                    |
| Dialogs         | **None** — every tab is inline panel content (provider add/edit/remove and encryption are inline rows/toggles)                                                                                                                                                                                                                                                                                                                    |
| Outgoing nav    | Index rows → `/settings/{connections,sync,privacy,ai,usage,preferences}` (some with a `?section=` sub-anchor) and kaiord.com/docs (Help & docs, new tab); rail entries → any section laterally; `/settings` (Back, `md:hidden`). `/settings/profile` is a separate AppRoutes redirect → `/athlete`                                                                                                                                |
...
| Outgoing nav | None internal — all edges are inbound: StatusHeader "Trends" → `/health`; StatusHeader "Labs" → `/health/labs` (own `nav-destinations.ts` entry, header-only surface); calendar WellnessBadge `<Link>` → specific sub-routes; ImportDropzoneOverlay post-save → `healthDestinationFor(type)`                                                                                                                                           |
...
| DataFlowsAddDialog (removed)                       | _deleted (was `organisms/ProfileManager/components/DataFlowsAddDialog.tsx`)_                       | None — retired in F4.2 for the Data Hub matrix, which was itself retired at the cutover; routing now lives on `/settings/connections`                                            | Add a sync source/destination (bridge + mode)                              | n/a (removed)                                          |
...
- **Global chrome:** `App` → `MainLayout` (LayoutHeader [HeaderLogo + StatusHeader (StatusEntryButtons, StatusIndicators, ProfileEntryButton, ThemeToggle)], StorageAvailabilityBanner, `<main>`, BottomNav [BottomNavTab ×4 + BottomNavFab]); sibling overlays MigrationBoot and AppKeyboardShortcuts. Header overlays (`useHeaderOverlays`): the ⌘K command palette and the `?` shortcut sheet. Hooks: `useFocusOnRouteChange`, `useRouteAnnouncerLabel`.
...
- **Athlete (`/athlete`):** AthletePage → AthletePageBody (AthleteIdentity, Segmented, ThresholdCard [ThresholdCardHeader, ThresholdMetricsRow, Metric], ZoneMapCard [ZoneMap]). Dialogs: ProfileEditDialog → ProfileManagerDialog/ProfileEditView (ProfileTabs, DeleteConfirmDialog), ThresholdEditDialog (SportZoneEditor), ConfirmationModal.
...
- **Settings (`/settings/:section?`):** SettingsPage → SettingsAttention + (SettingsGroupList [SettingsRow → SettingsRowBody] | SettingsSectionRail + section organism): AiTab (ProviderList → ProviderRow/ProviderEditRow, ProviderForm, ModelsSection/ModelPicker), SyncTab (SyncStatusLine, EncryptionSection), ConnectionsTab (ConnectionsHealth [ConnectionSummaryRow/Tile, ConnectionsBanner, ConnectionRefreshButton], ConnectionSourceCard [ConnectionCardHeader/ConnectionMark, ConnectionStatusLine, ConnectionBridgeLine, ConnectionRouteChips, ConnectionCardAction, ConnectionManagePanel → ConnectionRouteList/ConnectionSyncButton/ConnectionBodyExport/DisconnectConfirmation, ConnectionApiKeyPanel → ApiKeyConnectForm], DataTypeRoutingSection → DataTypeRoutingGroup/Row → RoutingFreshness/RoutingExportTargets/RoutingSourcePicker, ConnectionsUnsupported), UsageTab (UsageTable/UsageEmptyState), PrivacyTab (PrivacyInformationSection), PreferencesTab (LanguageRow, NotificationsRow). Registry: `settings-tab-views.tsx` (also owns the retired-section redirect map).
...
- **Three connection surfaces collapsed into one (`/settings/connections`):** the Data Flows tab went first (F4.2 — `useDataFlowsAdd`/`DataFlowsAddDialog`/`DataFlowsAddFields`, then `DataFlowsSection`/`Group`/`Subsection`/`Row`), superseded by the Data Hub matrix; `ProfileTabs` dropped its "data-flows" entry. The cutover then retired the matrix itself along with the Extensions tab and the Athlete page's Connections section: **deleted** `organisms/DataHub/**`, `SettingsPanel/ExtensionsTab.tsx`, `SettingsPanel/BridgeStatusRow.tsx`, `SettingsPanel/TanitaGarminSyncCard.tsx`, `organisms/AthleteConnections/{AthleteConnections,ConnectedRow,AvailableRow,ApiKeyRow,ApiKeyRowHeader,NotSupportedRow,connection-config,index}`, the five `hooks/data-hub/use-{data-hub-matrix,data-hub-toggle,data-hub-route-editor,source-policies,source-policy-editor}` hooks, and the `data-hub` i18n namespace (both locales). `application/data-hub/source-policy-rows.ts` went with them; its one surviving export is now `application/connections/order-sources.ts`. `AthleteConnections/` no longer exists — its five surviving files (`ConnectionMark`, `DisconnectConfirmation`, `ApiKeyConnectForm`, `data-flow-lookup`, `use-policy-toggle`) moved to `organisms/Connections/` and `hooks/connections/`. **`application/data-hub/**` survives**: `buildDataHubMatrix` + `dataHubCellState` are the derivation behind the `get_data_routes` chat tool, and `readBridgeSyncStates` feeds the routing rows and the connection model. The Tanita → Garmin body-composition push moved into the Tanita card's Manage panel (`ConnectionBodyExport`).
