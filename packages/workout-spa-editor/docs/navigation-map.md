...
> **Nav registry:** every navigation surface — the header bar, its "More"
> overflow menu, the avatar menu and the mobile bottom nav — derives from the
> single registry in `src/routing/nav-destinations.ts` (table in
> `nav-rows.ts`; parity test: `nav-destinations.test.ts`, which fails unless
> each destination has exactly ONE desktop surface). Do not add destinations
> to any surface directly.
...
| `/daily`                 | `DailyPage` (`components/pages/DailyPage`)                           | `?date=YYYY-MM-DD`                                                       | The former Today page                                                                                                 |
| `/today`                 | — (`routing/legacy-today-redirect.tsx`)                              | `?date=` preserved                                                       | **Legacy alias → `/daily`** (the page was renamed)                                                                    |
| `/athlete`               | `AthletePage` (`components/pages/AthletePage/AthletePage.tsx`)       | —                                                                        | Gates on active profile (spinner / empty / body)                                                                      |
| `/nutrition`             | `NutritionPage` (`components/pages/NutritionPage`)                   | —                                                                        | Intake / energy-balance surface                                                                                       |
| `/chat/:conversationId?` | `ChatPage` (`components/pages/ChatPage`)                             | `conversationId?`                                                        | No id → conversation list with an empty thread pane                                                                   |
...
| `/settings/profile`      | — (`AppRoutes.tsx`)                                                  | —                                                                        | **Redirect → `/athlete`** (declared above `/settings/:section?` so it wins)                                           |
| `/settings/:section?`    | `SettingsPage` (`components/pages/SettingsPage/SettingsPage.tsx`)    | `section?` ∈ ai\|sync\|data-hub\|extensions\|usage\|privacy\|preferences | No section → grouped index. Section → `md:` section rail beside the panel. Unknown section → **Redirect `/settings`** |
| `/health/*?`             | `HealthSubRouter` (`components/pages/health/health-routes.tsx`)      | trailing path                                                            | Sub-router: `/health`, `/sleep`, `/weight`, `/recovery`, `/activity`, `/labs`. Unknown → **Redirect `/health`**       |
...
graph LR
  subgraph HealthHub["Health hub (HealthSubRouter)"]
    ...
    HR["/health/recovery"]
    HA["/health/activity"]
    HL["/health/labs (DoD-1 entry form)"]
    HX["unknown /health/*"] -->|redirect| HD
  end
  STATUS["Header Trends dropdown"] -->|navigate /health| HD
  STATUSLABS["Trends dropdown -> Labs row"] -->|navigate /health/labs| HL
...
- **LayoutHeader** (`LayoutHeader.tsx`) — **sticky top-0 z-50**, present on every route. `HeaderLogo` (brand `<Link href="/calendar">`, left) + `StatusHeader` (right). `StatusHeader` is ONE nav row: `HeaderNavBar`, a divider, `SourceHealthPill`, `AccountMenu`.
  - **`HeaderNavBar`** renders `BAR_ENTRIES` (registry rows with the `bar` or `overflow` surface) in table order: Daily, Calendar, Library, Nutrition, Trends, Chat, Athlete, New workout. An entry with children becomes a `NavMenu` dropdown listing itself and them — that is Trends, whose only child is Labs. Below `lg` the `overflow` entries (Nutrition, Trends+Labs, Chat) leave the bar for a single "More" `NavMenu`; the Nutrition row inside it is `hidden md:flex` because the bottom nav already carries it on mobile.
  - Per-slot visibility (`BAR_WRAPPER_CLASS`, on a plain wrapper — `Button`'s base classes hardcode `inline-flex`, which would race `hidden` on the same element): `overflow` → `hidden lg:inline-flex`; bottom-nav- or FAB-covered → `hidden md:inline-flex`; otherwise always.
  - **`AccountMenu`** — avatar trigger (`status-header-account-button`, initials from `accountInitials`) opening the account-level destinations: Connections (`/settings/connections`, with an amber dot when a source needs attention), Settings (`/settings`), a "Help & docs" external link, and the `ThemeToggle`. The toggle is a `DropdownMenu.Item asChild` with `onSelect` prevented, NOT a plain nested button: Radix menus `preventDefault()` on Tab, so anything in the content that is not an item is keyboard-unreachable.
  - **`SourceHealthPill`** — amber, and rendered ONLY when `useHeaderAttention()` is non-null. There is no healthy variant. Both it and the avatar dot read `useConnectionAttention` (`hooks/connections/`), the same derivation the Settings banner reads.
  - There is **no Help button** — help is keyboard-first: `useHeaderOverlays` mounts the ⌘K command palette and the `?` shortcut sheet, and long-form documentation lives on the docs site. There are **no per-bridge status chips**: `StatusIndicators` was removed, as were `ProfileEntryButton` and `StatusEntryButtons`.
- **BottomNav** (`components/molecules/BottomNav/BottomNav.tsx`) — **MOBILE ONLY** (`md:hidden`), fixed floating glass bar (`inset-x-[14px] bottom-[14px] z-30`, `max-w-md`, rounded). **5 tabs** from `BOTTOM_NAV_TABS` (registry rows with `surfaces.bottomNav`): Daily → `/daily`, Calendar → `/calendar`, Library → `/library`, Nutrition → `/nutrition`, Athlete → `/athlete`; plus a center notch **FAB** → `/workout/new` (`CREATE_WORKOUT_PATH`) at `NOTCH_INDEX` 3. `isTabActive` sets `aria-current` (Calendar active on `/` too). **Desktop has NO bottom nav** — navigation is the header row only.
...
- **Global non-visual / overlay chrome** (App.tsx siblings of MainLayout): `MigrationBoot` (v10 migration toast), and `AppKeyboardShortcuts` (editor-store save/undo/redo/copy/paste/group — **not navigation**). The six-step centred `OnboardingTutorial` and its `AppTutorial` wrapper are gone; first-run guidance is now the live setup checklist plus anchored coach marks.

**Chrome per breakpoint.** `lg` (≥1024): the full bar — seven destinations plus New workout — with no "More" trigger and no bottom nav; `main` `md:pb-6`. `md`–`lg` (768–1023): Nutrition, Trends and Chat collapse into "More"; still no bottom nav, so "More" is the only way to reach them. Below `md`: the bar keeps only "More", the pill and the avatar (every other entry is bottom-nav- or FAB-covered), the floating BottomNav + FAB appear, and `main` gets `pb-28`. Trends, Labs and Chat are reachable on mobile through "More" — the bottom nav has no room for them, and the reference design's mobile header, which showed neither, would have stranded Trends and Labs.
...
| Sub-views    | loading / empty / body; sport Segmented (cycling\|running\|swimming, in-page useState); connected-bridge cards link out to the Data Hub routing matrix instead of expanding in place (F4.2) |
| Dialogs      | ProfileEditDialog (tabs: Training Zones / Personal Data / Linked Accounts; nested DeleteConfirmDialog), ThresholdEditDialog, disconnect ConfirmationModal                                   |
...
|                 |                                                                                                                                                                                                                                                                                                                                                                                                       |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Route           | `/settings/:section?` (section ∈ ai\|sync\|data-hub\|extensions\|usage\|privacy\|preferences)                                                                                                                                                                                                                                                                                                         |
| Renders         | `components/pages/SettingsPage/SettingsPage.tsx` — grouped index (`SettingsGroupList`) when no section; otherwise `SettingsSectionRail` + the section organism from the `settings-tab-views.tsx` registry. The two are mutually exclusive, so `settings-row-*` ids exist only on the index                                                                                                            |
| Header          | Global header + in-page "Settings" h1 (route heading, exactly one at any URL); a section adds a tertiary `md:hidden` Back button + "Settings · <Section>" h1                                                                                                                                                                                                                                          |
| Bottom nav      | Visible (mobile)                                                                                                                                                                                                                                                                                                                                                                                      |
| Responsive      | **Yes, from `md`** — with a section open the container is `md:grid md:grid-cols-[18rem_minmax(0,1fr)]`: a `hidden md:block` section rail (`SettingsSectionRail`, one entry per section, `aria-current="page"` on the open one) sits beside the panel and the index list is not rendered. CSS only; no viewport is measured. Not sticky (see the change's design.md D9)                                |
| Sub-views       | Index groups (Your data / AI / Preferences / About); sections: AI (ProviderList + inline ProviderEditRow + ProviderForm + custom prompt), Sync (status line + Connect/Sync/Disconnect + EncryptionSection), Data Hub (routing matrix), Extensions (bridge status table + Refresh), Usage (6-month table), Privacy (info + Clear All API Keys, no confirm), Preferences (units/language/notifications) |
| Attention slots | `SettingsAttention` banner (under the h1) and chip (top of the rail) are mounted but fed `null`, so neither renders until a connection-health model exists                                                                                                                                                                                                                                            |
| Dialogs         | **None** — every tab is inline panel content (provider add/edit/remove and encryption are inline rows/toggles)                                                                                                                                                                                                                                                                                        |
| Outgoing nav    | Index rows → `/settings/{ai,sync,data-hub,extensions,usage,privacy,preferences}` (some with a `?section=` sub-anchor), `/athlete` (Connections, until Wave 1 gives it its own section) and kaiord.com/docs (Help & docs, new tab); rail entries → any section laterally; `/settings` (Back, `md:hidden`). `/settings/profile` is a separate AppRoutes redirect → `/athlete`                           |
...
| Renders      | 6 lazy pages: `/health` (Dashboard/Trends), `/health/sleep`, `/health/weight`, `/health/recovery`, `/health/activity`, `/health/labs` (`labs/LabEntryPage.tsx`); unknown → Redirect `/health`                                                                                                                                                                                                                                          |
...
| Sub-views    | Dashboard is the only interactive page: TrendMetricSelector (multi-select chips, default sleep+hrv), TrendRangeSelector (30/90/365, default 90), TrendSingleChartCard. `/health/labs` is a DoD-1 manual entry form (report header + N add/remove parameter rows, catalog autocomplete or free parameter, live unit conversion) — gated on an active profile. Other 4 pages are read-only Dexie lists/stats with loading + empty states |
| Dialogs      | None (no dialog/overlay/sheet in the area)                                                                                                                                                                                                                                                                                                                                                                                             |
| Outgoing nav | None internal — all edges are inbound: the header's Trends dropdown → `/health`, and its Labs row → `/health/labs` (own `nav-rows.ts` entry, `nested` under `trends`); calendar WellnessBadge `<Link>` → specific sub-routes; ImportDropzoneOverlay post-save → `healthDestinationFor(type)`                                                                                                                                           |
...
| ProfileManagerDialog / ProfileEditView             | `organisms/ProfileManager/components/*`                                                            | ProfileEditDialog                                                                                                                                                                | Tabbed profile editor (zones/personal/linked)                              | No                                                     |
...
| DataFlowsAddDialog (removed)                       | _deleted (was `organisms/ProfileManager/components/DataFlowsAddDialog.tsx`)_                       | None — retired in F4.2: creating a route is now exclusively the Data Hub matrix's job (`/settings/data-hub`)                                                                     | Add a sync source/destination (bridge + mode)                              | n/a (removed)                                          |
| Data Flows tab (removed)                           | _deleted (was `organisms/ProfileManager/components/DataFlowsSection\|Group\|Subsection\|Row.tsx`)_ | None — the whole tab was retired in F4.2 once the Data Hub matrix covered both create/toggle (cell click) and mode-edit/remove (per-route menu)                                  | List + edit (mode, enabled, remove) existing sync routes                   | n/a (removed)                                          |
...
| LayoutHeader / StatusHeader                | **Yes**                         | Overflow entries `hidden lg:inline-flex` + "More" `lg:hidden`; bottom-nav/FAB duplicates `hidden md:inline-flex`; entry labels `hidden md:inline`; pill label `hidden lg:inline`; divider `hidden md:inline-block` | md, lg             |
...
| `/settings/:section?`                      | **Yes**                         | Index is single-column at all widths; with a section open the index becomes a `md:grid-cols-[18rem_1fr]` rail beside the panel (back button `md:hidden`)                                                           | md (768)           |
...
| `/health/labs`                             | **Yes**                         | Report header fieldset `grid-cols-1` → `sm:grid-cols-2`; parameter rows are a single flex-wrap row at all widths                                                                                                   | sm                 |
...
- **Global chrome:** `App` → `MainLayout` (LayoutHeader [HeaderLogo + StatusHeader (HeaderNavBar [EntryButton ×n + NavMenu for Trends and More], SourceHealthPill, AccountMenu [AccountMenuItems → ThemeToggle])], StorageAvailabilityBanner, `<main>`, BottomNav [BottomNavTab ×5 + BottomNavFab]); sibling overlays MigrationBoot and AppKeyboardShortcuts. Header overlays (`useHeaderOverlays`): the ⌘K command palette and the `?` shortcut sheet. Hooks: `useFocusOnRouteChange`, `useRouteAnnouncerLabel`.
...
- **Athlete (`/athlete`):** AthletePage → AthletePageBody (AthleteIdentity, Segmented, ThresholdCard [ThresholdCardHeader, ThresholdMetricsRow, Metric], ZoneMapCard [ZoneMap], AthleteConnections [ConnectedRow/AvailableRow — ConnectedRow links out to the Data Hub matrix instead of expanding per-flow toggles, F4.2]). Dialogs: ProfileEditDialog → ProfileManagerDialog/ProfileEditView (ProfileTabs, DeleteConfirmDialog), ThresholdEditDialog (SportZoneEditor), ConfirmationModal.
...
- **Settings (`/settings/:section?`):** SettingsPage → SettingsSidebar → SettingsGroupList (SettingsRow) / section organisms in `SettingsPanel`: AiTab (ProviderList → ProviderRow/ProviderEditRow, ProviderForm, ModelSelect), SyncTab (SyncStatusLine, EncryptionSection), ExtensionsTab (BridgeStatusRow), UsageTab (UsageTable/UsageEmptyState), PrivacyTab (PrivacyInformationSection). Registry: `settings-tab-views.tsx`.
- **Health (`/health/*`):** HealthSubRouter → HealthDashboardPage (HealthPageHeader, TrendMetricSelector, TrendRangeSelector, TrendSingleChartCard → UplotChart), HealthSleepPage, HealthWeightPage, HealthRecoveryPage, HealthActivityPage, `labs/LabEntryPage` (HealthPageHeader, `LabEntryForm` → `LabReportHeaderFields`, `LabParameterRow` → `LabParameterIdentityField` + `LabParameterMeasurementFields`; hook `use-lab-entry-form`). Hooks: `useTrendSelection`, `useTrendSeries`, `health-date-windows`. Entry helpers: WellnessBadge/WELLNESS_BADGE_ROUTES, `healthDestinationFor`.

## Coverage notes

- **Routing ground truth verified** against `AppRoutes.tsx`: all **16** declared routes are present in the route table above and correctly attributed, including the `/calendar` → `/calendar/:weekId` week-grid redirect and the `/settings/profile` → `/athlete` redirect being declared _above_ `/settings/:section?` so it wins. (`/daily`, `/today`, `/nutrition` and `/chat/:conversationId?` were added to the table on 2026-07-29 — the count previously read "all 12" and had been stale since those routes shipped. They are in the table but do not yet have their own screen block below.) The Health sub-router's 6 lazy pages (including `/health/labs`, added for the lab-analytics DoD-1 entry form) + null→Redirect fallback are all mapped.
- **Data Flows tab retired in full (F4.2):** first the "+ Add source/destination" dialog (`useDataFlowsAdd`/`DataFlowsAddDialog`/`DataFlowsAddFields`) went, once the Data Hub matrix (`/settings/data-hub`) covered the same create-by-(dataType, bridge, direction) surface; then the whole tab (`DataFlowsSection`/`DataFlowsGroup`/`DataFlowsSubsection`/`DataFlowsRow`) followed once the matrix ALSO gained a per-route mode-edit/remove menu (`DataHubCellMenu`) — closing the one gap (editing mode, removing a route) the matrix didn't originally cover. `ProfileTabs` dropped the "data-flows" entry; `ProfileEditView` no longer renders it. AthleteConnections' per-connection "What syncs" toggles (`ConnectionFlows`/`FlowRow`/`flow-availability.ts`/`use-bridge-capabilities.ts`) were retired the same way — `ConnectedRow` now only links to the matrix.
...
- **`/settings/profile` is now caller-less (overhaul):** the two former `navigate("/settings/profile")` call sites were removed — the header's Athlete entry navigates to `/athlete` directly, and `AthleteEmptyState` opens `ProfileManagerDialog` in place. The `/settings/profile` → `/athlete` redirect route is kept only for backward-compatible external links.
