...
/* Today is the one filled pill on the row; the rest of the header stays flat,
   so the accent is spent once instead of on a column-wide wash. */
const TODAY_PILL = "rounded-full bg-accent px-2.5 py-0.5 text-surface";
...
export function CalendarWeekGridHeader({
  ...
  return (
    <div
      ...
      className="sticky top-0 z-10 mb-2 hidden bg-surface-page sm:grid sm:grid-cols-7 sm:gap-2"
    >
      {days.map((date) => {
        ...
        return (
          <div
            ...
            className="flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-medium text-ink-muted"
          >
            ...
            {count >= MULTI_WORKOUT_THRESHOLD && (
              <span
                ...
                className="ml-auto rounded-full bg-surface-elevated px-1.5 py-0.5 text-[10px] font-medium tabular-nums text-ink-body"
              >
