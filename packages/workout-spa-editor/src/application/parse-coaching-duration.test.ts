...
describe("parseCoachingDuration", () => {
  it("parses plain minutes (various spellings)", () => {
  ...
  it("parses hours-and-minutes (both spellings)", () => {
  ...
  it("parses cycling apostrophe notation", () => {
  ...
  it("drops the approximate marker", () => {
    expect(parseCoachingDuration("~45 min")).toBe(2700);
    expect(parseCoachingDuration("~1h 30m")).toBe(5400);
  });

  it("uses the lower bound for ranges (minutes)", () => {
    expect(parseCoachingDuration("45-50 min")).toBe(2700);
    expect(parseCoachingDuration("60-90min")).toBe(3600);
  });

  it("uses the lower bound for ranges (hours)", () => {
  ...
  it("returns undefined for ISO 8601 range syntax (not supported v1)", () => {
    expect(parseCoachingDuration("PT45M/PT50M")).toBeUndefined();
    expect(parseCoachingDuration("PT45M--PT50M")).toBeUndefined();
  });

  it("is deterministic for canonical forms", () => {
