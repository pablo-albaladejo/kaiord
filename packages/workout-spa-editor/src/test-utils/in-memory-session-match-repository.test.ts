...
describe("InMemorySessionMatchRepository", () => {
  ...
  it("getByActivityId is profile-scoped", async () => {
    const repo = createInMemorySessionMatchRepository();
    await repo.put(makeMatch({ id: "M1", profileId: "p1" }));

    expect(
      await repo.getByActivityId("p2", "p1:train2go:12345")
    ).toBeUndefined();
  });

  it("rejects double-match on activity side", async () => {
    const repo = createInMemorySessionMatchRepository();
  ...
  it("upsert by id is idempotent (re-put same id replaces)", async () => {
    const repo = createInMemorySessionMatchRepository();
    await repo.put(makeMatch({ id: "M1", source: "manual" }));

    await repo.put(makeMatch({ id: "M1", source: "auto-suggestion" }));
  ...
  it("listByProfileAndWeek inclusive on both ends, profile-scoped", async () => {
    const repo = createInMemorySessionMatchRepository();
    ...
    const results = await repo.listByProfileAndWeek(
      "p1",
      "2026-04-27",
      "2026-05-03"
    );

    expect(results.map((m) => m.id).sort()).toEqual(["M1", "M2"]);
  });
  ...
  it("deleteByActivityId removes only matching rows, idempotent on miss", async () => {
    ...
    expect((await repo.getByActivityId("p1", "p1:train2go:other"))?.id).toBe(
      "M2"
    );

    await expect(
      repo.deleteByActivityId("nonexistent")
    ).resolves.toBeUndefined();
  });

  it("deleteByWorkoutId removes only matching rows, idempotent on miss", async () => {
    const repo = createInMemorySessionMatchRepository();
    await repo.put(makeMatch({ id: "M1", workoutId: "w-keep" }));
    await repo.put(
      makeMatch({
        id: "M2",
        coachingActivityId: "p1:train2go:other",
    ...
    expect(await repo.getByWorkoutId("p1", "w-keep")).toBeDefined();
