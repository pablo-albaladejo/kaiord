...
export const unresolvedTargetFallbackEvent = (
  targetKind: "item" | "empty-state",
  fallback: "empty-state" | "first-item" | "heading"
): FocusTelemetryEvent => ({ type: "unresolved-target-fallback", targetKind, fallback });

export const formFieldShortCircuitEvent = (): FocusTelemetryEvent => ({
  type: "form-field-short-circuit",
});

export const overlayDeferredApplyEvent = (measuredMs: number): FocusTelemetryEvent => ({
...
export const safeEmit = (telemetry: FocusTelemetry, event: FocusTelemetryEvent): void => {
  try {
    telemetry(event);
  } catch {
    console.warn("[FocusTelemetry] telemetry handler threw — focus behavior unaffected", event);
  }
};
