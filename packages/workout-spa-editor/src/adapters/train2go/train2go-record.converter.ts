/**
 * Train2Go Record Converter — Wire → Record.
 *
 * Named `*.converter.ts` (per project convention) because it carries
 * derivation logic (workload→intensity rounding, status fallback)
 * beyond a simple field-by-field mapping.
 *
 ...
 *   -1 → "skipped"
 *
 * intensity: clamp(round(workload), 1, 5) when workload > 0; undefined
 * otherwise. The raw `workload` is preserved verbatim (NOT clamped, NOT
 * rounded) for analytics that need the platform-native signal.
 *
