/**
 * v27 → v28 migration — provenance backfill + partial policy seeding.
 *
 ...
 *      (health imports, dual-write activities) already carry a real source,
 *      so re-running only touches rows that still lack one. Workouts keep
 *      their existing `source` column (no `sourceBridgeId` field applies).
 *   2. Seed a default enabled `planned-session` import policy for every profile
 *      with an active Train2Go link, so that once the train2go sync is gated
 *      on that policy a currently-syncing profile keeps syncing untouched.
 *      This is a PARTIAL advance of the fail-open seeding (which will add
