...
- 744a78f: Replace the inline 7-band Coggan power-zone table in `packages/zwo/src/adapters/target/power.converter.ts` with the canonical domain helper `zoneToPercentFtp` from `@kaiord/core` (added in W6.1). Public API of `@kaiord/zwo` is byte-identical — `convertPowerZoneToPercentFtp` is internal and unchanged at the call sites in `power-encoder.ts`.
