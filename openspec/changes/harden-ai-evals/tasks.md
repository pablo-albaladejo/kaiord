...
- [ ] B1. Reject a `zoneCheck` whose bounds the comparison cannot use, as a
      ...
      rejected in W4.
- [ ] B2. Give `zones-ftp` the bounds its own prompt states — `minValue: 88`,
      `maxValue: 93` — turning B1 green. Land after B1, not with it.
- [ ] B3. Fix `assertions.ts:72`: no matching active steps must not be a silent
      pass. Return a named failure, or `Unmeasured` where that is the honest
      answer.
- [ ] B3b. Fix the twin defect in the same function: `assertions.ts:82-95`
...
- [ ] B4. Invert `assertions.test.ts:274`
...
- [ ] **C1a — delete the two lines.** Documentation-only; cannot fail
      mechanically. Correct if per-dimension gating was never wanted.
- [ ] **C1b — implement them.** Per-dimension results out of `evaluateBenchmark`
...
- [ ] C3. Fix the invalid model id `claude-sonnet-4-5-20241022` in
...
- [ ] D1. Remove the floors from `run-evals.ts`, `run-chat-tool-evals.ts` and
...
- [ ] D2. Each runner states at its head that it cannot run in this project, why
      (no provider key; `load-eval-model.ts:33`), and what would change that.
- [ ] D3. Fix `packages/ai/README.md:67`, which documents a ≥90% gate as live.
      ...
      all.
- [ ] D4. Carry the declaration to the two surfaces a user touches first: the
...
- [ ] E4. Replace `chat-tool-assertions.ts:28,52`'s `?? {}` / `?? []` with a
...
- [ ] E5. Name every rate E2 introduces for its scale, and keep rounding on the
...
- [ ] F1. Keyless containment suite: fence breakout, opening delimiter,
      concatenation re-formation, and the cap under a delimiter-heavy payload.
- [ ] F2. `scripts/check-untrusted-fields-fenced.mjs` — every recognized
      externally-authored field returned by a model-facing summarizer must be
      assigned from a `fenceUntrusted(...)` call.
- [ ] F3. Its co-located `node:test` must contain: a negative fixture that
...
- [ ] F4. Declare the recognized field-name set in one place, stating that the
      guard is a syntactic allowlist and does not see opaque nested payloads or
      error text in action-tool results.
- [ ] F5. Declared sink inventory over every value-returning `execute:` in
      ...
      never "pending".
- [ ] F5b. Each entry must name **where each returned string was produced**, not
...
- [ ] F6. Add `error`, `message` and `reason` to the recognized field set —
      externally-originated error strings are the class that produced the only
      live channel found.
- [ ] F7. `summarize-coaching.ts:21,23` declares `title: string` /
...
- [ ] F8. Fix `check-ai-sdk-containment.mjs:56`'s `catch { return violations }`,
      which reports green when it cannot read the directory.
- [ ] F9. Wire the new guard into `lint:parallel`. `test:scripts` needs no
      wiring — it is a fixed glob and collects the test automatically.

## G. Close-out

- [ ] G1. Settle whether imported workout names are externally authored.
