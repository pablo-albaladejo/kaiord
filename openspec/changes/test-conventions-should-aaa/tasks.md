...
- [ ] 1.10 Co-localized invariants verification + empirical smoke checks. Run `pnpm test:scripts` and `pnpm lint`. Both green. Run `pnpm -r test`: all packages green (no test changed). Then perform three additional empirical smoke checks and record the results in the PR-1 description:
