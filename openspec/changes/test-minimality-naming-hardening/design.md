...
- TCX running-cadence semantics (D4): **RESOLVED during implementation.** Investigation of the wired path found the situation is worse than the audit stated: the live reader (`tcx-target-walker.converter.ts`) decodes only `HeartRate_t` and degrades `Cadence_t`/`Speed_t` targets to `open` with a warning, so cadence and pace targets are lossy on the live round-trip today. The live writer also matches pace unit `"meters_per_second"`, which is not a `targetUnitSchema` member (canonical is `"mps"`), so single-value pace targets fall through to `None_t`. Decision: port the orphan chain's documented, symmetric semantics to the wired path — running cadence is SPM in TCX and RPM in KRD (SPM = 2 × RPM; writer doubles, reader halves, sport-aware on both legs; other sports pass through), and the pace unit is the canonical `targetUnitSchema.enum.mps`. Sport is threaded from the workout converters into the step/target converters on both legs.
