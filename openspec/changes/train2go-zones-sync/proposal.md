...
- `bodyWeight` and `heartRate.max` are derivable from the existing `/profile/ping` payload (`data.user.weight`, `data.user.bpm_max`). They are populated by the same use case but require **no new endpoint** — the bridge already fetches them.
