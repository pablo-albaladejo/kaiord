...
- [x] 2.3 Add a `parseDetailsHtml` function to `packages/train2go-bridge/parser.js` that takes the inline HTML and extracts a stable `ZonesPayload` (`{ physiological: { weight, height, bpmMax, bpmRest, fatPct? }, hr: { z1..z5 lower/upper }, paces: { running, swimming, cycling }, tests: { ftp?, ... }, records: { ftp?, ... } }`). Use stable DOM IDs and `name=` attributes — never visible labels. Mirrors the existing `parseWeeklyHtml` pattern.
- [ ] 2.4 Add a `readDetails()` function to `packages/train2go-bridge/background.js` that calls `train2goFetch("/user/details")` and pipes the HTML through `parseDetailsHtml`. Returns `{ ok, data: ZonesPayload }`.
...
- [x] 2.6b Redaction unit test for `parseDetailsHtml`. Feed a fixture containing forbidden fields (gender, birthday, fat, smoker, bpm_rest, user_notes, coach.email, coach.name, email, records, tests). Assert none of those field names appear in `JSON.stringify(output)`.
