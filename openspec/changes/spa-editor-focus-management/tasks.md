...
- [ ] 3.1.c Parameterize a conversion-export test across formats the SPA uses (KRD + any adapters wired in): assert no `id` values leak into the conversion port input. The runtime assertion in 3.1.a is the primary enforcement. Additionally, IF task 3.3 determines `@kaiord/core`'s `KRD` Zod schema uses `.strict()` (so `id` is a structurally-rejected property), THEN add a compile-time type assertion `expectTypeOf<ReturnType<typeof stripIds>>().not.toHaveProperty('id')` on an arbitrary step/block picked from the returned workout. If the KRD schema is permissive (allows unknown properties), the `expectTypeOf` check is not meaningful at the type level and SHALL be skipped — the runtime structural-equality assertion already documented in 3.3 is the enforcement mechanism
...
- [ ] 10.3.a Add CI grep invariant for narrow-selector discipline on `pendingFocusTarget` consumers. The rough helper `grep -R "useWorkoutStore(\s*)" packages/workout-spa-editor/src | grep -v "pendingFocusTarget\|useWorkoutStore((s)"` helps locate wide `useWorkoutStore` subscriptions during local audits; it is NOT the enforcement. The actual enforcement lives in `scripts/check-pending-focus-selectors.mjs` (wired via task 11.9) which asserts that every file referencing `pendingFocusTarget` uses a narrow selector `(s) => s.pendingFocusTarget` OR carries an explicit `// allow-wide-selector: <reason>` codeword comment immediately above the wide subscription
