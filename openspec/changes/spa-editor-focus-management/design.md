...
- **[Risk] Render loop if something sets `pendingFocusTarget` again in response to the clear** → Mitigation: hook clears *before* applying focus (batched into the same update), so a downstream re-set is the next batch, not a loop. Scenario covered in spec.
