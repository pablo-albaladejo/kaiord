...
- **The rail is the index, not a copy of it.** The same list component renders
  in both roles under a `variant`. As the rail it drops the inline values and
  the version footer, because the open section's own panel is already the place
  those values are stated — and because two copies of a row would make every
  row test id ambiguous.
- **The open section is marked** on the rail with `aria-current="page"` and a
  tint, resolved by comparing the row's destination path with the current one.
...
- **Routing**: `AppRoutes.tsx` renames the settings route parameter `:tab?` →
  ...
  changes.
- **New files**: `SettingsSidebar.tsx`, `SettingsAttention.tsx` and
...
- **e2e**: no test id and no URL changes. `settings-row-<key>` and
  `settings-panel-<id>` keep their identity; the rail deliberately omits row
  values so provider labels stay unique on the page under Playwright's strict
  mode.
