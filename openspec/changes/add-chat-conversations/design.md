...
*Alternative considered:* a separate per-conversation `aiModelBindings`-style store. Rejected: the override is 1:1 with a conversation row, so it belongs on that row — no extra store, no extra sync surface.
...
*Alternative considered:* persist an empty row on "New". Rejected: accumulates titleless rows that sync as noise (the OQ3 risk).
