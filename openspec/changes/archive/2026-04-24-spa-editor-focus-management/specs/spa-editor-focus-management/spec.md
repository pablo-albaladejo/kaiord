...
- **GIVEN** `selectionHistory` records the selection immediately before a reversed add/paste/duplicate, and the recorded selected item is still present in the restored workout
...
- **GIVEN** no recorded prior selection exists or the recorded item is no longer present in the restored workout
...
The workout store SHALL maintain `selectionHistory: Array<ItemId | null>` parallel to `workoutHistory`. Every time a mutation pushes a new `UIWorkout` snapshot, the pre-mutation `selectedStepId` (the primary single-selection id, not `selectedStepIds`) SHALL be pushed to `selectionHistory` at the same index. `selectionHistory` SHALL be used by undo actions to compute `pendingFocusTarget` per the rules above. `clearWorkout` SHALL reset `selectionHistory` to an empty array in the same state update.

#### Scenario: Selection snapshot pushed on every mutation

- **WHEN** any mutating action (delete, add, paste, duplicate, group, ungroup, reorder) pushes a new snapshot to `workoutHistory`
- **THEN** `selectionHistory.length` SHALL equal `workoutHistory.length` and the last entry SHALL equal the `selectedStepId` value immediately before the mutation

#### Scenario: Undo reads matching selection snapshot

- **WHEN** `undo` decrements `historyIndex` to `i`
- **THEN** the pre-mutation selection that can be consulted for focus fallback SHALL be `selectionHistory[i + 1]` (the selection at the time the undone mutation ran)

#### Scenario: Clear workout resets selection history

- **WHEN** `clearWorkout` is called
- **THEN** `selectionHistory` SHALL be reset to an empty array
- **AND** `workoutHistory` SHALL also be reset so the length invariant is preserved
...
- **THEN** `selectionHistory` SHALL push `selectedStepId` (the primary single-selection id), not the full `selectedStepIds` array
...
`pendingFocusTarget` and `selectionHistory` SHALL NOT span a Dexie reload. Upon reloading a workout from Dexie, the workout store SHALL initialize `pendingFocusTarget` to `null`, `selectionHistory` to empty, and regenerate all `ItemId` values via the configured `IdProvider`. Every Dexie write SHALL strip `id` fields via a single `stripIds` helper so that no `ItemId` values are persisted.
