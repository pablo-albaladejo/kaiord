...
- [x] 1.3 Add an explanatory comment above the `check-links` job noting why it is always on and why lychee is pinned.

## 2. Config hygiene

- [x] 2.1 Add a comment to `lychee.toml` explaining that `include_fragments` is intentionally omitted — the setting changed from boolean to string enum in lychee 0.24 and the default (no fragment validation) is correct for this repo's offline/internal link check. (Shipped in #348.)
...
- [x] 3.2 Run `pnpm lint:specs` locally; fix any structural-lint findings.
- [x] 3.3 Run `npx openspec validate changes/harden-link-checker` if available.
...
- [x] 4.1 Add `Link checker` to required status checks:
  ```sh
...
- [x] 5.1 Confirm Link checker runs on the PR itself (it will, since removing the `if:` means it always runs).
- [x] 5.2 Confirm the PR passes CI (all required + Link checker).
- [ ] 5.3 After post-merge steps, open a trivial follow-up test PR and confirm `Link checker` appears under "Required" in the merge box. (Deferred — not blocking; protection state verified via API in 4.3.)
