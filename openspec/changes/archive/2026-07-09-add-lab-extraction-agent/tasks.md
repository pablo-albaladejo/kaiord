...
- [x] 1.1 Define the port and events in `packages/ai/src/observability/`:
...
- [x] 2.1 Write the deterministic smoke-lane suites FIRST (red) in
...
- [x] 2.2 Implement `definition-types.ts`: `AgentDefinition<TOutput>` =
...
- [x] 2.3 Implement `prelude.ts` (resolve system prompt via the registry,
      stamp traceId/latency, emit telemetry around the run) and
      `build-user-message.ts` (text + file content parts → AI SDK message).
- [x] 2.4 Implement `generate-mode.ts` + `retry-policy.ts` by generalizing
...
- [x] 2.5 Export `runGenerateAgent(definition, input, config)` where config =
...
- [x] 3.1 Define the `workout-parser` `AgentDefinition` (id
...
- [x] 3.2 Rewire `createTextToWorkout` as a deprecated thin wrapper over the
...
- [x] 3.3 Generalize the manual eval loader: `load-eval-model.ts` reads
...
- [x] 4.1 Add `lab-extractor/system` to the prompt registry (v `1.0.0`,
...
- [x] 4.2 Define the permissive `labExtractionSchema` (report header +
      values array of `{ label, parameterKey?, value?, unit?, refLow?,
refHigh?, refText? }`) with unit tests; keep it flat (structured-output
      complexity limits).
- [x] 4.3 Define the `lab-extractor` `AgentDefinition` (purpose
      ...
      unaffected).
- [x] 4.4 Smoke-lane suite for the extractor on `MockLanguageModelV4`
...
- [x] 5.1 Thread provenance through the builders: `build-lab-report.ts` /
...
- [x] 5.2 Implement `application/lab/extraction/map-extraction-to-draft.ts`:
...
- [x] 5.3 Implement `application/lab/extraction/run-lab-extraction.use-case.ts`:
...
- [x] 6.1 `LabImportSection` on the entry tab: file input (accept
...
- [x] 6.2 Wire the draft into the existing form: extraction result pre-fills
...
- [x] 6.3 Add en/es i18n strings for all new UI copy; extend the settings
...
- [x] 7.1 `pnpm -r test && pnpm -r build && pnpm lint` green across the
      monorepo; file/function caps respected; coverage thresholds met
      (80% core packages / 70% frontend); `lint:ai-sdk-containment` green.
- [x] 7.2 `pnpm lint:specs` green (this change's delta specs validate).
- [x] 7.3 Verify chat is untouched: no diff under `packages/ai/src/chat/`
      and no change to `spa-ai-chat` behavior/suites.
- [x] 7.4 Add the single changeset: `@kaiord/ai` minor (`./agents` +
      `./observability` subpaths, lab-extractor agent, eval lane).
- [x] 7.5 Evidence run (manual, needs API keys): `pnpm eval` before/after
