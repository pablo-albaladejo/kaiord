...
The script SHALL follow the project's repo-script convention: entry-point check via `pathToFileURL(process.argv[1]) === import.meta.url`, exported pure functions for testability, structured one-line log lines for greppable workflow output. Tests SHALL mock the Octokit boundary and exercise every branch enumerated in the requirements below.

#### Scenario: Workflow invokes the script

- **WHEN** the `notify-failure` job runs on a red main build
- **THEN** the job's main step SHALL be `node scripts/ci-failure-issue.mjs create '<failed-jobs-json>'`; no inline JS performs Octokit calls

#### Scenario: Tests cover all decision branches

- **WHEN** `pnpm test:scripts` runs
- **THEN** the suite SHALL include at least 11 tests for `ci-failure-issue.mjs` covering create, comment-dedupe, close-on-match, skip-on-job-mismatch, missing-footer, malformed-footer, race-closed, unknown-schema, absent-schema-back-compat, in-process dedupe, and no-op-on-no-issue
...
- **GIVEN** an open `ci,automated` issue created before footer versioning shipped (no HTML-comment footer present)
...
#### Scenario: Legacy issue without footer is not auto-closed
...
- **GIVEN** an open issue with footer `failed-jobs: ["lint"]` and no `schema:` line
- **WHEN** the close-pass runs against a green run that covered `lint`
- **THEN** the bot SHALL parse as schema 1 and SHALL close the issue
...
The close-pass SHALL perform an immediately-pre-close staleness re-check by re-fetching the issue via Octokit. If the issue was closed by another process between the `list` call and the pre-close `get`, the close-pass SHALL skip with `skipped: race-closed` and log accordingly.
...
- **WHEN** both jobs reference concurrency group `ci-issue-bot-main`
...
The first step of each job SHALL log either "ci-failure-bot enabled" or "ci-failure-bot disabled via vars.CI_ISSUE_BOT_ENABLED" so the kill-switch state is visible in workflow logs.
...
- **THEN** `notify-failure` SHALL be skipped (the job's `if:` evaluates false); SHALL log "ci-failure-bot disabled via vars.CI_ISSUE_BOT_ENABLED"; no issue SHALL be created

#### Scenario: Variable absent or set to anything else enables both jobs

- **GIVEN** `vars.CI_ISSUE_BOT_ENABLED` is unset OR set to any string other than `false`
- **WHEN** a red main build triggers the workflow
- **THEN** `notify-failure` SHALL run; SHALL log "ci-failure-bot enabled"

### Requirement: Canary path with synthetic-failure fallback

Phase 1 SHALL be considered shipped only after the DRI has inspected one bot-created issue end-to-end. The canary SHALL be either (a) the first natural red-main run after merge, inspected within 24h, OR (b) a synthetic failure forced by triggering the dedicated `ci-issue-bot-canary` workflow via `workflow_dispatch` after 7 days of green main.
...
- **GIVEN** Phase 1 was merged and a red main run occurred within 24h
- **WHEN** the bot creates an issue
- **THEN** the DRI SHALL inspect the issue body, footer marker, labels, and the corresponding workflow log within 24h; the inspection acceptance SHALL be recorded as a comment on the merged Phase 1 PR

#### Scenario: Synthetic canary fires after 7 days of green main

- **GIVEN** Phase 1 was merged and main has been green for 7 days
- **WHEN** the DRI triggers the `ci-issue-bot-canary` workflow via `workflow_dispatch`
- **THEN** the bot SHALL create an issue titled `[CANARY] 🚨 CI Failure on main branch` with labels `ci,automated,canary` and footer `failed-jobs: ["canary-job"]`, `schema: 1`; the DRI SHALL inspect and manually close it; Phase 2 MAY then proceed

#### Scenario: Canary issue is not auto-closed by real green runs

- **GIVEN** an open canary issue with footer `failed-jobs: ["canary-job"]`
- **WHEN** any subsequent real green main run completes (which executes the real CI job set, none of which is named `canary-job`)
- **THEN** the close-pass SHALL log `skipped: jobs not covered by this run` and SHALL NOT close the canary issue; only manual close by the DRI resolves it
