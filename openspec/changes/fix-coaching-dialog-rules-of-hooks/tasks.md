...
- [x] 1.2 In the same SPA-files block, add `no-restricted-imports` (or `@typescript-eslint/no-restricted-imports`) entries restricting `coaching-registry-context` and `coaching-source` modules for files matching the three dialog files (verified to exist today: `CoachingActivityDialog.tsx`, `CoachingActivityDialogContent.tsx`, `use-coaching-dialog.ts`). The rule MUST list:
  - All relative-path spellings that resolve to the target modules from the dialog files (currently `../../contexts/coaching-registry-context` and `../../types/coaching-source`).
...
- [x] 1.4a `react-hooks/exhaustive-deps` blast-radius bound: if activating the rule produces MORE than 5 findings across the SPA, downgrade `react-hooks/exhaustive-deps` from `error` to `warn` for THIS change, open a follow-up issue ("re-enable exhaustive-deps as error"), and document the downgrade + the finding count in the PR. ≤ 5 findings: fix in-place per zero-tolerance. This bound prevents the change from ballooning while keeping the structural goal (rules-of-hooks must be `error`) untouched.
...
- [ ] 6.2 Create `scrub-analytics-string.test.ts` covering:
...
- [ ] 6.4 Update `RouteErrorBoundary.test.tsx` to assert ONLY the React-integration concerns:
...
- [ ] 6.5 Run `pnpm -F @kaiord/workout-spa-editor test`. All error-boundary tests MUST pass. Run `wc -l RouteErrorBoundary.tsx` and confirm ≤ 80 lines.
