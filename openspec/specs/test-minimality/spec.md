> Synced: 2026-06-11 (test-minimality-naming-hardening)

# Test Minimality

## Purpose

Defines the minimal-and-semantic test-suite invariants for every Vitest suite
in the monorepo: no suites against pure re-export barrels (mechanically
guarded by `R-NoBarrelTestSuite`), round-trip-named files exercise both
conversion legs, every test asserts an observable outcome, dispatcher suites
assert routing only, and titles describe domain behavior above the
implementation.
