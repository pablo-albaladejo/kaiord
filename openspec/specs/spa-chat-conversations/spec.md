> Synced: 2026-06-21 (add-chat-conversations)

# SPA Chat Conversations

## Purpose

Multi-conversation management for the in-SPA AI chat assistant: per-profile conversation entities with auto-generated editable titles, a conversation list, new-conversation drafts, per-conversation model overrides, single-conversation deletion with tombstones, and cross-device sync — so the user can keep distinct, deep-linkable threads instead of one rolling transcript.
