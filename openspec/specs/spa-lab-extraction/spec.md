...
# spa-lab-extraction Specification

## Purpose

Defines the labs V2 document-extraction feature: upload a PDF or photo, extract with the user's own provider, review the AI-extracted draft in the existing entry form, and save with `ai-extracted` provenance. Documents are transit-only and printed report ranges keep authority.
