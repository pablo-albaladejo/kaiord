...
- **Communication scope**: Each extension only communicates with its declared host (`connect.garmin.com` / `app.train2go.com` / `mytanita.eu`) and allowed Kaiord origins. The Kaiord-origin channel (`externally_connectable`) SHALL be described as one-way inbound (editor → extension)
